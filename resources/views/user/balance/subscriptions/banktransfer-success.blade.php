					...
					<div class="text-center">
						...
						@if ($id->plan_type == 'subscription')
							<div class="text-center mb-6">
								...
								<p class="fs-14">{{ __('After successful payment, each month you will have') }} <span class="text-info font-weight-bold">{{ number_format($id->minutes) }}</span> {{ __('minutes added to your account. To keep your subsciption active in coming month, please provide payments by the end of the current month.') }}.</p>
						...
						@if ($id->plan_type == 'prepaid')
							<div class="text-center mb-5">
								<p class="mt-5 fs-14">{{ __('You have successfully placed order for new ') }} <span class="text-info font-weight-bold">{{ number_format($id->minutes) }}</span> {{ __('minutes for your account') }}.</p>
								@if ($id->bonus > 0)
									<p class="fs-14">{{ __('With') }} <span class="text-info font-weight-bold">+{{ number_format($id->bonus) }}</span> {{ __('bonus minutes as well') }}.</p>
								@endif	
								<p class="fs-14">{{ __('Please provide payment to our bank requisites below. Use Order ID number as payment reference') }}.</p>	
								<p class="text-muted fs-12 mb-4">Order ID: <span class="font-weight-bold text-primary">{{ $orderID }}</span></p>
