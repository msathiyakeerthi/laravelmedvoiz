					...
					<div class="text-center">
						...
						@if ($plan->plan_type == 'subscription')
							<div class="text-center mb-6">
								...
								<p class="fs-14">{{ __('Each month you will have') }} <span class="text-info font-weight-bold">{{ number_format($plan->minutes) }}</span> {{ __('minutes added to your account') }}.</p>
							</div>						
						@endif
						@if ($plan->plan_type == 'prepaid')
							<div class="text-center mb-5">
								<p class="mt-5 fs-14">{{ __('You have successfully added') }} <span class="text-info font-weight-bold">{{ number_format($plan->minutes) }}</span> {{ __('minutes to your account') }}.</p>
								@if ($plan->bonus > 0)
									<p class="fs-14">{{ __('Bonus') }} <span class="text-info font-weight-bold">+{{ number_format($plan->bonus) }}</span> {{ __('minutes were added as well') }}.</p>
								@endif								
							</div>						
						@endif
						<div class="text-center pt-2 pb-4">
							...
							<a href="{{ route('user.stt.file') }}" id="payment-button" class="btn btn-primary pl-6 pr-6">{{ __('Start Usage') }}</a>							
						</div>
					</div>
				</div>
			</div>
